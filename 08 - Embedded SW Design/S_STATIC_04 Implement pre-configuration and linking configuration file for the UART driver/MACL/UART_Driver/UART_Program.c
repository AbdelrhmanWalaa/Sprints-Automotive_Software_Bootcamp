...
*/
u8 UART_u8ReceiveByteBlock( u8 *Cpy_pu8ReturnedReceiveByte )
{
	/* Define local variable to set the error state = OK */
	u8 Loc_u8ErrorState = STD_TYPES_OK;

	/* Check 1: Pointer is not equal to NULL */
	if ( Cpy_pu8ReturnedReceiveByte != NULL )
	{
        /* Step 1: Wait ( Poll ) until Byte is Received ( i.e. until Flag ( RXC ) = 1 ) */
